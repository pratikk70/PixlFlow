...
const handlePopup = async () => {
  try {
    // 1. If user is NOT signed in → open Clerk Sign-in popup
    if (!isSignedIn) {
      if (window.Clerk && window.Clerk.openSignIn) {
        return window.Clerk.openSignIn();
      }
      return console.error("Clerk sign-in popup unavailable");
    }

    // 2. If user already has this plan → do nothing
    if (isCurrentPlan) return;

    // 3. User is signed in → open Checkout
    ...
    console.error("Checkout or Sign-in error:", error);
  }
};
...
const PricingSection = () => {
  const plans = [
    ...
    },
    {
      ...
      featured: true,
      planId: "cplan_363JEt75tUWI6FBctHowQtxvHIA",
